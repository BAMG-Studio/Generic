...
### Option A – EC2 Baseline (**active**)

- Terraform continues to provision shared AWS services (IAM, S3, CloudTrail, CloudWatch) while the runtime lives on an EC2 host running the existing Docker Compose stack (`forgetrace-postgres`, `forgetrace-mlflow`, optional `nginx`).

- Pros: fastest path (reuse current compose + `deployment/nginx/nginx.conf`), minimal AWS cost (~$5–10/mo), straightforward secret injection via `.env` and GitHub Secrets, easy rollback through AMI snapshots.

- Cons: single-host footprint (manual scaling/HA), TLS management remains manual until ACM/ALB is added, and patching responsibility stays with the ops team.

### Option B – Kubernetes Expansion (**future**)

- Requires container image publishing plus manifests/Helm for MLflow, nginx ingress, and secret management (ConfigMaps/Secrets). Pros: horizontal scaling, rolling upgrades, native ingress TLS. Cons: higher operational overhead, new Terraform/EKS modules, and duplicated secret distribution.

> Action: Production stays on **Option A** until EKS parity (databases, artifact storage, ingress, secrets) is engineered and reviewed.
...
- **Owner:** ForgeTrace_Security_Team is accountable for rotation execution, runbooks, and audit evidence.
