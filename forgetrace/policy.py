...
class PolicyEngine:
    ...
    def _validate_policy_schema(self, policy: Dict[str, Any]) -> bool:
        ...
        if action not in [a.value for a in PolicyAction]:
            print(
                f"❌ Policy '{policy['id']}': Invalid action '{action}'. "
        ...
        if "severity" in policy:
            ...
            if severity not in [s.value for s in PolicySeverity]:
                print(
                    f"❌ Policy '{policy['id']}': Invalid severity '{severity}'. "
    ...
    def _evaluate_policy(
        self, policy: Dict[str, Any], findings: Dict[str, Any]
    ) -> bool:
        """
    ...
    def _build_evaluation_context(self, findings: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if "classification" in findings:
            ...
            total = (
                len(context["foreground_files"])
                + len(context["background_files"])
                + len(context["third_party_files"])
            )
            if total > 0:
                context["third_party_percentage"] = (
                    len(context["third_party_files"]) / total
                ) * 100
            else:
    ...
    def _create_violation(
        ...
        message = policy.get(
            "description", f"Policy '{policy.get('name', policy['id'])}' was violated"
        )

        # Add condition for debugging
        details = {"condition": policy["condition"], "policy_id": policy["id"]}
    ...
    def _print_summary(self) -> None:
        ...
        print(f"\n{'=' * 60}")
        print("Policy Violations Summary:")
