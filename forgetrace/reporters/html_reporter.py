...
<body>
    <div class="container">
        ...
        <p><strong>Tool:</strong> {{ sbom.tool|default('fallback') }}</p>
        <p><strong>Packages inventoried:</strong> {{ sbom.packages|length if sbom.packages else 0 }}</p>
        {% if sbom.exports %}
        <p><strong>Download detailed SBOM files:</strong></p>
        <ul>
            {% for export in sbom.exports %}
            <li>
                <a href="{{ export.path }}">{{ export.format }}</a>
                ({{ export.size_bytes|default(0)|format_number }} bytes)
            </li>
            {% endfor %}
        </ul>
        {% else %}
        <p>No CycloneDX/SPDX exports captured for this run.</p>
        {% endif %}
    ...
    def __init__(
        self, findings: Dict[str, Any], output_dir: str | Path, config: Dict[str, Any]
    ) -> None:
    ...
    def generate(self) -> None:
        ...
        # Create environment with autoescaping to mitigate XSS in reports
        env = Environment(
            autoescape=select_autoescape(
                default=True, enabled_extensions=("html", "xml")
            )
        )
