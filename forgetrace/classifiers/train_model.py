...
def evaluate_model(model: RandomForestClassifier, X: np.ndarray, y: np.ndarray) -> None:
    ...
    for i, label in enumerate(labels):
        print(f"{label[:4]:>6}", "  ".join(f"{cm[i][j]:>6}" for j in range(len(labels))))
...
def save_model(model: RandomForestClassifier, output_path: str = "models/ip_classifier.pkl") -> None:
    """
    ...
    - ONNX: Language-agnostic, no pickle security issues
    - joblib: Optimized for large numpy arrays
    - MLflow: Model versioning + metadata tracking
...
def main():
    ...
    """
    if len(sys.argv) < 2:
        print("Usage: python -m forgetrace.classifiers.train_model <training_data.jsonl>")
        print("\nExample:")
