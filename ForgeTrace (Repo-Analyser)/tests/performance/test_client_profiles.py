...
# Test configuration
PROFILING_DATASETS_DIR = Path(__file__).parent.parent.parent / "training" / "profiling_datasets"
...
def get_profile_paths() -> list:
    ...
    if not catalog:
        pytest.skip("No client profiles configured in catalog")
...
@pytest.mark.parametrize("profile_id,profile_dir", get_profile_paths())
def test_client_profile_performance(profile_id: str, profile_dir: Path):
    ...
    expected_third_party = expected.get('ip_breakdown', {}).get('third_party_percentage', 0)
...
def test_all_profiles_exist():
    ...
    for profile_id in catalog.keys():
        ...
        assert (profile_dir / "metadata.yaml").exists(), \
            f"metadata.yaml missing for {profile_id}"
        
        assert (profile_dir / "samples").exists(), \
            f"samples/ directory missing for {profile_id}"
...
def test_catalog_integrity():
    ...
    for profile_id, profile_data in catalog.items():
        ...
        # Validate data types
        assert isinstance(profile_data['files'], int), \
            f"'files' must be int for {profile_id}"
        
        assert isinstance(profile_data['size_mb'], (int, float)), \
            f"'size_mb' must be numeric for {profile_id}"
        
        assert isinstance(profile_data['characteristics'], list), \
