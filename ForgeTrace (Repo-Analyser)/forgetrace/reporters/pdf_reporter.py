...
class PDFReporter:
	...
	SECTION_ORDER: Dict[str, List[str]] = {
		"board": ["cover", "narrative", "kpis", "risk", "compliance", "rewrite", "next_steps"],
		"executive": [
			"cover",
			"narrative",
			"kpis",
	...
	def __init__(self, findings: Dict[str, Any], output_dir: Path | str, config: Dict[str, Any] | None) -> None:
	...
	def generate(self) -> None:
		...
		except TemplateNotFound as exc:  # pragma: no cover - hard failure during runtime usage
			raise RuntimeError(
		...
		try:
			...
			HTML(string=rendered_html, base_url=str(html_path.parent)).write_pdf(str(pdf_path))  # type: ignore[attr-defined]
	...
	def _build_context(self) -> Dict[str, Any]:
		...
		vulnerabilities = cast(Dict[str, Any], self.findings.get("vulnerabilities", {}) or {})
		policy_findings = cast(List[Dict[str, Any]], self.findings.get("policy_violations", []) or [])
		...
		provenance = self._build_provenance_insights(git_info, classifications, top_authors)
		rewrite_stats = self._summarize_rewriteability(rewriteability, classifications)
		rewrite_stats["quadrants"] = self._build_rewrite_quadrants(classifications, rewriteability)
		...
		stakeholder_questions = self._build_stakeholder_questions(risk, compliance, rewrite_stats)
		engagement = self._build_engagement_section(output_cfg, stakeholder_questions)
		appendix = self._build_appendix_section(dependencies, risk, stakeholder_questions)
		next_steps = self._build_next_steps(risk, rewrite_stats, compliance)

		kpis = self._build_kpis(cost, dependencies, compliance, rewrite_stats, classifications, vulnerabilities)
		...
		recommendation = self._decide_recommendation(kpis, compliance, rewrite_stats, policy_summary)
		...
		similarity_section = self._summarize_similarity_section(similarity, duplicate_highlights)
		dependency_observations = self._build_dependency_observations(dependencies, compliance)
		top_packages = self._identify_top_packages(cast(List[Dict[str, str]], dependencies.get("items", [])))

		narrative_blocks = build_narrative(kpis, risk, rewrite_stats, compliance, recommendation, vulnerabilities)
		commit_sparkline = self._build_commit_sparkline(cast(List[Dict[str, Any]], git_info.get("timeline", []) or []))
		...
		branding = {
			...
			"watermark": metadata.get("watermark_text") or output_cfg.get("watermark_text") or "",
			"classification_label": metadata.get("classification_label") or output_cfg.get("classification_label") or "",
		}
		engagement_id = metadata.get("engagement_id") or output_cfg.get("engagement_id") or ""
		classification_label = branding["classification_label"] or "Confidential - Technical Due Diligence"
	...
	def _build_kpi_cards(self, mode: str, kpis: Dict[str, Any]) -> List[Dict[str, str]]:
		cards = [
			{"label": "Open Vulnerabilities", "value": self._format_int(kpis.get("vuln_open_count", 0))},
			{
				"label": "Vuln Density",
			...
			{"label": "Foreground LOC", "value": self._format_int(kpis.get("loc_foreground", 0))},
			{"label": "Background LOC", "value": self._format_int(kpis.get("loc_background", 0))},
			{"label": "Third-Party Components", "value": self._format_int(kpis.get("components_third_party", 0))},
			{"label": "High-Risk Licenses", "value": self._format_int(kpis.get("licenses_high_risk", 0))},
			{"label": "Clean-Room Candidates", "value": self._format_int(kpis.get("clean_room_candidates", 0))},
			{"label": "Foreground Share", "value": self._format_percent(kpis.get("foreground_share", 0.0))},
			{"label": "Third-Party Share", "value": self._format_percent(kpis.get("third_party_share", 0.0))},
			{"label": "Total LOC Inventoried", "value": self._format_int(kpis.get("loc_total", 0))},
		]
	...
	def _build_negotiation_points(
		...
		if kpis.get("background_share", 0.0) >= 20.0:
			points.append(
				f"Background IP share sits at {self._format_percent(kpis['background_share'])}; confirm contract scope covers these modules."
			)

		if int(compliance.get("high_risk_count", 0) or 0) > 0:
			points.append("Reciprocal licenses present; negotiate carve-outs or plan clean-room isolation.")

		if int(rewrite.get("low_rewriteable_count", 0) or 0) > 0:
			points.append("Low rewriteability foreground modules require clean-room planning before partner disclosures.")

		if kpis.get("third_party_share", 0.0) >= 60.0 and len(points) < 3:
			points.append(
				"Third-party code exceeds 60% of inventoried files; expect diligence questions on integration rights."
			)

		if not points and key_risks and key_risks.lower() != "no material risks flagged in this pass":
			points.append(key_risks)

		if not points:
			points.append("No material blockers detected; maintain watchlist as new code lands.")
	...
	def _build_license_heatmap(
		...
		for category in ("Permissive", "Weak Copyleft", "Strong Copyleft"):
			row = buckets.get(category, {"foreground": 0, "background": 0, "third_party": 0})
			heatmap.append(
				[
					category,
	...
	def _extract_high_risk_identifiers(self, license_summary: List[Dict[str, Any]]) -> List[str]:
		high_risk: List[str] = []
		for entry in license_summary:
			...
			if any(keyword in upper_name for keyword in ("GPL", "AGPL", "SSPL", "COPYLEFT")):
				high_risk.append(name)
		return high_risk[:6]

	def _build_ip_share_list(self, classifications: Dict[str, Dict[str, Any]]) -> List[Dict[str, str]]:
		counts, total = self._count_origins(classifications)
		if total == 0:
			return [
		...
		return [
			{"label": "Foreground", "value": self._format_percent((counts.get("foreground", 0) / total) * 100)},
			{"label": "Background", "value": self._format_percent((counts.get("background", 0) / total) * 100)},
			{"label": "Third-party", "value": self._format_percent((counts.get("third_party", 0) / total) * 100)},
		]

	def _select_top_modules(self, contribution_table: List[Dict[str, Any]]) -> List[Dict[str, Any]]:
	...
	def _summarize_policy_violations(self, policy_findings: List[Dict[str, Any]]) -> Dict[str, Any]:
	...
	def _summarize_similarity_section(
		...
		public_matches_raw = cast(List[Dict[str, Any]], similarity.get("public_matches", []) or [])
		...
		duplicates = duplicate_highlights[:5] or [{"file1": "-", "file2": "-", "similarity": "0.00"}]
		...
		if public_matches:
			summary.append("Public code matches detected; confirm provenance and licensing for overlapping files.")
		else:
	...
	def _build_rewrite_quadrants(
		...
		for path, metrics in rewriteability.items():
			...
			difficulty = "Easy" if score >= 0.75 else "Hard" if score <= 0.4 else "Medium"
			origin_raw = str(classifications.get(path, {}).get("origin", "foreground")).lower()
			risk = "Low-risk"
		...
		for key in ("Easy/Low-risk", "Easy/High-risk", "Hard/Low-risk", "Hard/High-risk"):
	...
	def _count_origins(self, classifications: Dict[str, Dict[str, Any]]) -> Tuple[Counter[str], int]:
	...
	def _summarize_authors(self, git_info: Dict[str, Any]) -> Tuple[List[Dict[str, Any]], str]:
		...
		if rows:
			...
			top_rows = [{"name": "Unknown", "commits": 0, "added": 0, "removed": 0, "files": 0}]
	...
	def _summarize_rewriteability(
		...
		low_rewriteable = [
			...
			and str(classifications.get(path, {}).get("origin", "")).lower() == "foreground"
		]
		high_rewriteable = [
			path
			for path, meta in rewriteability.items()
		...
		priority_modules = [Path(path).name or path for path in low_rewriteable[:5]] or ["No critical modules identified."]
	...
	def _build_next_steps(
		...
		secrets_exposure = next(
			(int(item.get("count", 0)) for item in risk.get("items", []) if item.get("area") == "Secrets"),
			0,
		)
		sast_findings = next(
			(int(item.get("count", 0)) for item in risk.get("items", []) if item.get("area") == "SAST"),
			0,
		)
	...
	def _build_compliance_obligations(self, license_summary: List[Dict[str, Any]]) -> Dict[str, Any]:
		...
		if high_risk_count:
			obligations.insert(
				0,
				f"Review {high_risk_count} GPL-family component obligation{'s' if high_risk_count != 1 else ''} for distribution compatibility.",
			)
	...
	def _build_commit_sparkline(self, timeline: List[Dict[str, Any]]) -> Dict[str, Any] | None:
		if not timeline:
			return None
		...
		for entry in timeline:
			...
			count_raw = entry.get("commits") or entry.get("count") or entry.get("value") or 1
			try:
				count = int(count_raw)
			except (TypeError, ValueError):
				count = 1
